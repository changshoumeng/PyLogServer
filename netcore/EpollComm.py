        ...
        try:
            ...
            while not is_sigint_up:
                try:
                    ...
                    netLogger.critical("taskloop>>" + "*" * 15)
                    info = sys.exc_info()
                    for file, lineno, function, text in traceback.extract_tb(info[2]):
                        s1 = "taskloop>>file:{0} line:{1} in function:{2}".format(file, lineno, function)
                        netLogger.critical(s1)
                        s2 = "taskloop>>** %s: %s" % info[:2]
                        netLogger.critical(s2)
                    netLogger.critical("taskloop>>" + "#" * 15)
            else:
                netLogger.critical("taskloop>>catched interrupt signal!!!")

        except Exception, e:
            message = "taskloop>>try_serve_once caught a exception:{0}".format(repr(e))
            netLogger.critical(message)
            sys.exit(0)
        finally:
            self.worker.stop()
...
class ConnectionBase(object):
    ...
    def __init__(self, client_session_id=-1, client_socket=None, client_addr=()):
        ...
        self.session_uuid = "{0}_{1}_{2}".format(self.connection_type,
    ...
    def onReadEvent(self):
        ...
        while self.connect_status == CONNECT_STATUS.CONNECT_SUCC:
            try:
                ...
                elif 0 < processed_size < len(self.recv_buffer):
    ...
    def onDisconnectEvent(self):
        if self.connect_status == CONNECT_STATUS.CONNECT_CLOSED:
            return
        netLogger.warning("onDisconnectEvent|%s,reason:%d", self.session_uuid, self.connect_status)
