...
class EpollClient(ServerInterface):
    ...
    def start(self):
        ...
        for i in xrange(self._connection_pool_size):
            connector = _Connector(i + 1, None, self._connect_addr)
...
class _Connector(ConnectionBase):
    ...
    def _process_recv_buffer(self):
        pid = str(os.getpid())
        print "_Connector recv:", len(self.recv_buffer)
        # print "sendbytes:{0} recvbytes:{1}".format(self.send_bytes, self.recv_bytes)
        # self.sendData(self.recv_buffer)
